...
csvlogger = csv.writer(open("alexnet_single_stage_kd.csv", 'w'))
#csvlogger = csv.writer(open("vgg11_single_stage_kd.csv", 'w'))

# -------------------------------------------------------
def write_to_csv(step_data):
    with open('dcq_alexnet_stage1_loss.csv', 'a') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(step_data)
# csv file initialization:
headers = ['int_loss']
with open('dcq_alexnet_stage1_loss.csv', 'w') as writeFile:
...
def main():
    ...
    if args.compress:
        ...
        print("=======================================(Model)======")
        print(type(model))
        original_compression_scheduler = distiller.CompressionScheduler(original_model)
        ...
        pretrained_dict = {k: v for k, v in model.features.module.state_dict().items() if 'float' not in k}
        print(pretrained_dict.keys())
        # 2. overwrite entries in the existing state dict
        model.features.module.state_dict().update(pretrained_dict) 
        # 3. load the new state dict
        model.load_state_dict(pretrained_dict)
        print(model.features.module.state_dict().keys())
        """

        """ 
        model_state_dict = model.features.module.state_dict()
        original_model_state_dict = original_model.features.module.state_dict()
        print(model_state_dict.keys())
        KEYS_1 = model_state_dict.keys()
        KEYS_list = [each for each in KEYS_1]
        print('initial number of keys = ', len(KEYS_list))
        for each in KEYS_list:
           if 'float' in each:
              print(each)
              del model_state_dict[each]
              del model.features.module.state_dict()[each]
              print('removing key:', each)
        print((model.features.module.state_dict().keys()))
        model.features.module.state_dict().keys() = model_state_dict.keys() 
        #model.load_state_dict(model_state_dict)
        #del model.features.module.state_dict()['0.float_weight']
        #model.features.module.state_dict()['0.float_weight']=[]
        print((model.features.module.state_dict().keys()))
        print((model.features.module))
        #print(model.features.module.state_dict().keys())
        print("=======================================(Clean keys)======")
        print(original_model_state_dict.keys())
        """
...
def train(train_loader, model, original_model, criterion, optimizer, epoch,
    ...
    for train_step, (inputs, target) in enumerate(train_loader):
        ...
        if args.kd_policy is None:
            ...
            """ debug """
            #print("==================================")
            #print(model.features.module[0].weight)
        ...
        if not args.earlyexit_lossweights:
            ...
            """ different losses """
            #new_criterion = nn.L1Loss(size_average = True) 
            #new_criterion = nn.L1Loss(size_average=None, reduce=None, reduction='mean')

            #new_criterion = nn.MSELoss(size_average = True) 
            new_criterion = nn.MSELoss(size_average=None, reduce=None, reduction='mean')

            #new_criterion = nn.KLDivLoss()
            #new_criterion = nn.PoissonNLLLoss() #nn.PoissonNLLLoss() #nn.L1Loss() #torch.nn.KLDivLoss() #torch.nn.MSELoss(size_average = False) 
            #new_criterion = nn.CrossEntropyLoss(weight=None, size_average=None, ignore_index=-100, reduce=None, reduction='mean')

            #loss = torch.mean(torch.abs(new_tensor - old_tensor))  # MAE
            #loss = torch.mean((new_tensor - old_tensor)**2)  # MSE

            delta = 0.4
            loss1 = torch.abs(new_tensor - old_tensor)  
            loss2 = (new_tensor - old_tensor)**2  
            loss1[loss1 < delta] = 0
            loss2[loss2 > delta] = 0
            #loss = torch.mean(torch.add(loss1, loss2))  # Huber 

            #if train_step in [20,  40,  140]:
            #if epoch == 32:
            #if not (train_step%10):
               #np.save('fmaps0/test3_q_act_'+str(train_step)+'_'+str(epoch), new_tensor.data.cpu().numpy())
               #np.save('fmaps0/test3_ref_act_'+str(train_step), old_tensor.data.cpu().numpy())

            loss = new_criterion(new_tensor, old_tensor)

            #write_to_csv([loss.item()])
            #print('loss >>>>>> ', loss)
            """ Measure accuracy and record loss """
            #classerr.add(output_new.data, target)
    ...
    write_to_csv([loss.item()])
    #np.save('q_act'+str(epoch), new_tensor.data.cpu().numpy())
    #np.save('ref_act'+str(epoch), old_tensor.data.cpu().numpy())
